	...
	<xsl:function name="testng:formatDuration">
		...
		<xsl:if test="$durationMs &lt; 86400000">
			...
			<xsl:if test="($durationMs > 60000) and ($durationMs mod 3600000 > 1000)">
				<xsl:value-of
					select="format-number(($durationMs mod 3600000) div 60000, '#')" />
				m
			</xsl:if>
			<!--Seconds -->
			<xsl:if test="($durationMs > 1000) and ($durationMs mod 60000 > 1000)">
				<xsl:value-of select="format-number(($durationMs mod 60000) div 1000, '#')" />
				s
			</xsl:if>
		</xsl:if>
		...
		<xsl:result-document href="{testng:absolutePath('navigation.html')}"
			...
			<html xmlns="http://www.w3.org/1999/xhtml">
				...
				<body>
					...
					<div>
						<a href="../logs/output.log" target="content"
							onclick="javscript:clearAllSelections();">Output Log
						</a>
					</div>
					<div>
						<xsl:for-each select="$suiteElements">
			...
			<tr>
				<td colspan="4" class="detailsBox">
					<div id="{$detailsId}" class="testMethodDetails">
						...
						<xsl:if test="./params">
							<xsl:if test="contains(./params/param[1],'screenshot')">
								<xsl:call-template name="formField">
									<xsl:with-param name="label" select="'ScreenShot'" />
									<xsl:with-param name="value" select="./params/param[2]" />
								</xsl:call-template>
							</xsl:if>
						</xsl:if>
						<xsl:if test="./params">
							<xsl:if test="not(contains(./params/param[1],'screenshot'))">
								<xsl:call-template name="formField">
